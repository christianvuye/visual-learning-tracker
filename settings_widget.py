...
class SettingsWidget:
    ...
    def create_general_settings(self):
        ...
        scrollbar = ttk_bs.Scrollbar(general_frame, orient=VERTICAL, command=canvas.yview)
        ...
        user_section = ttk_bs.LabelFrame(scrollable_frame, text="User Preferences", padding=15)
        ...
        self.username_var = tk.StringVar(value=self.settings.get('username', 'Learning Enthusiast'))
        ttk_bs.Entry(user_frame, textvariable=self.username_var, width=30).pack(side=LEFT, padx=(10, 0))
        ...
        self.default_category_var = tk.StringVar(value=self.settings.get('default_category', 'General'))
        ...
        ttk_bs.Label(autosave_frame, text="Auto-save (minutes):", width=20).pack(side=LEFT)
        self.autosave_var = tk.IntVar(value=self.settings.get('autosave_interval', 5))
        autosave_spin = ttk_bs.Spinbox(autosave_frame, from_=1, to=60, textvariable=self.autosave_var, width=28)
    ...
    def create_appearance_settings(self):
        ...
        # Theme section
        theme_section = ttk_bs.LabelFrame(appearance_frame, text="Theme & Colors", padding=15)
        ...
        theme_combo = ttk_bs.Combobox(
            ...
            values=["superhero", "darkly", "cyborg", "vapor", "cosmo", "flatly", "litera", "minty"],
            width=27,
            state="readonly"
        )
        ...
        window_section = ttk_bs.LabelFrame(appearance_frame, text="Window Settings", padding=15)
        window_section.pack(fill=X, padx=20, pady=(0, 20))
        
        # Start maximized
        self.start_maximized_var = tk.BooleanVar(value=self.settings.get('start_maximized', False))
        ttk_bs.Checkbutton(
            window_section,
        ...
        self.remember_position_var = tk.BooleanVar(value=self.settings.get('remember_position', True))
        ttk_bs.Checkbutton(
            window_section,
        ...
        self.show_sidebar_var = tk.BooleanVar(value=self.settings.get('show_sidebar', True))
        ttk_bs.Checkbutton(
            window_section,
    ...
    def create_study_settings(self):
        ...
        study_section = ttk_bs.LabelFrame(study_frame, text="Study Preferences", padding=15)
        ...
        ttk_bs.Label(duration_frame, text="Default Session (min):", width=20).pack(side=LEFT)
        self.study_duration_var = tk.IntVar(value=self.settings.get('default_study_duration', 25))
        duration_spin = ttk_bs.Spinbox(duration_frame, from_=5, to=120, textvariable=self.study_duration_var, width=28)
        ...
        ttk_bs.Label(break_frame, text="Break Duration (min):", width=20).pack(side=LEFT)
        self.break_duration_var = tk.IntVar(value=self.settings.get('break_duration', 5))
        break_spin = ttk_bs.Spinbox(break_frame, from_=1, to=30, textvariable=self.break_duration_var, width=28)
        break_spin.pack(side=LEFT, padx=(10, 0))
        
        # Notification settings section
        notification_section = ttk_bs.LabelFrame(study_frame, text="Notifications", padding=15)
        ...
        # Sound notifications
        self.sound_notifications_var = tk.BooleanVar(value=self.settings.get('sound_notifications', False))
        ttk_bs.Checkbutton(
            notification_section,
        ...
        self.daily_reminders_var = tk.BooleanVar(value=self.settings.get('daily_reminders', True))
        ttk_bs.Checkbutton(
            notification_section,
        ...
        analytics_section = ttk_bs.LabelFrame(study_frame, text="Analytics & Tracking", padding=15)
        ...
        self.auto_complete_var = tk.BooleanVar(value=self.settings.get('auto_complete_modules', False))
        ttk_bs.Checkbutton(
            analytics_section,
    ...
    def create_data_settings(self):
        ...
        # Backup section
        backup_section = ttk_bs.LabelFrame(data_frame, text="Backup & Export", padding=15)
        ...
        self.backup_location_var = tk.StringVar(value=self.settings.get('backup_location', '~/Documents/LearningTracker_Backups'))
        location_entry = ttk_bs.Entry(location_frame, textvariable=self.backup_location_var)
        ...
        # Database section
        database_section = ttk_bs.LabelFrame(data_frame, text="Database Management", padding=15)
        ...
        ttk_bs.Label(info_frame, text=f"Database: {os.path.basename(db_path)}").pack(anchor=W)
        ttk_bs.Label(info_frame, text=f"Size: {db_size}").pack(anchor=W)
        ttk_bs.Label(info_frame, text=f"Location: {os.path.dirname(os.path.abspath(db_path))}").pack(anchor=W)
    ...
    def create_about_tab(self):
        ...
        # About section
        about_section = ttk_bs.LabelFrame(about_frame, text="Application Information", padding=20)
        ...
        app_info = [
            ...
            ("", "", 8, 'normal'),
            ("A comprehensive learning management application", "Description", 11, 'normal'),
        ...
        for feature in features:
            ttk_bs.Label(features_frame, text=feature, font=('Helvetica', 10)).pack(anchor=W, pady=1)
    ...
    def preview_theme(self, event=None):
        ...
        messagebox.showinfo("Theme Change", "Theme will be applied when you restart the application.")
    ...
    def create_backup(self):
        ...
        messagebox.showinfo("Backup", "Backup functionality would copy the database to the specified location.")
        
    def export_data(self):
        """Export data to various formats"""
        messagebox.showinfo("Export", "Data export functionality would allow exporting to JSON, CSV, etc.")
        
    def import_data(self):
        """Import data from file"""
        messagebox.showinfo("Import", "Data import functionality would allow importing from backup files.")
    ...
    def optimize_database(self):
        ...
        messagebox.showinfo("Optimize", "Database optimization would run VACUUM and ANALYZE commands.")
    ...
    def reset_database(self):
        ...
        if messagebox.askyesno(
            ...
            if messagebox.askyesno(
                ...
                messagebox.showinfo("Reset", "Database reset functionality would reinitialize the database.")
