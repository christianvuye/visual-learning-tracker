...
class ExercisesWidget:
    ...
    def create_exercises_list(self):
        ...
        category_combo = ttk_bs.Combobox(
            ...
            values=["all", "coding", "algorithms", "system_design", "debugging", "refactoring", "testing"],
            state="readonly",
            width=15
        )
        category_combo.pack(side=LEFT, padx=(5, 0))
        category_combo.bind("<<ComboboxSelected>>", lambda e: self.refresh_exercises_list())
        ...
        # Scrollbar
        scrollbar = ttk_bs.Scrollbar(list_container, orient=VERTICAL, command=self.exercises_tree.yview)
    ...
    def create_exercise_info(self, parent):
        ...
        ttk_bs.Label(desc_frame, text="Description:", font=('Helvetica', 10, 'bold')).pack(anchor=W)
        self.description_text = tk.Text(desc_frame, height=3, font=('Helvetica', 9), state='disabled')
    ...
    def create_notes_section(self, parent):
        ...
        notes_scrollbar = ttk_bs.Scrollbar(notes_frame, orient=VERTICAL, command=self.notes_text.yview)
    ...
    def load_exercise_details(self, exercise_id: int):
        ...
        self.info_labels['estimated_time'].config(text=f"{exercise['estimated_time']} min")
        self.info_labels['actual_time'].config(text=f"{exercise['actual_time'] or 0} min")
        self.info_labels['status'].config(text=exercise['status'].replace('_', ' ').title())
        ...
        self.description_text.insert(1.0, exercise['description'] or 'No description available.')
    ...
    def save_notes(self):
        ...
        messagebox.showinfo("Info", "Note saving functionality to be implemented in database layer.")
    ...
    def open_conversation_link(self):
        ...
        exercises = self.db.get_exercises()
        exercise = next((ex for ex in exercises if ex['id'] == self.current_exercise_id), None)
        ...
        if exercise and exercise['conversation_link']:
            ...
            messagebox.showwarning("No Link", "No conversation link available for this exercise.")
    ...
    def delete_exercise(self):
        if not self.current_exercise_id:
            messagebox.showwarning("No Selection", "Please select an exercise to delete.")
            return
        
        if messagebox.askyesno("Confirm Delete", "Are you sure you want to delete this exercise?"):
            messagebox.showinfo("Delete", "Exercise deletion functionality to be implemented.")
...
class ExerciseDialog:
    ...
    def create_form(self):
        ...
        # Title
        ttk_bs.Label(main_frame, text="Create New Exercise", style='Header.TLabel').pack(pady=(0, 20))
        
        # Basic info
        basic_frame = ttk_bs.LabelFrame(main_frame, text="Basic Information", padding=10)
        basic_frame.pack(fill=X, pady=(0, 10))
        
        # Exercise title
        self.title_var = tk.StringVar()
        ttk_bs.Label(basic_frame, text="Exercise Title:").pack(anchor=W)
        ttk_bs.Entry(basic_frame, textvariable=self.title_var, width=60).pack(fill=X, pady=(0, 10))
        ...
        category_combo = ttk_bs.Combobox(
            ...
            values=["coding", "algorithms", "system_design", "debugging", "refactoring", "testing"],
            width=20
        )
        ...
        ttk_bs.Entry(diff_frame, textvariable=self.time_var, width=10).pack(side=LEFT, padx=(5, 0))
        ...
        course_combo = ttk_bs.Combobox(platform_frame, textvariable=self.course_var, state="readonly", width=25)
        ...
        ttk_bs.Entry(link_frame, textvariable=self.link_var, width=70).pack(fill=X, pady=(5, 0))
        ...
        ttk_bs.Entry(tags_frame, textvariable=self.tags_var, width=70).pack(fill=X, pady=(5, 10))
        
        ttk_bs.Label(tags_frame, text="Concepts (comma separated):").pack(anchor=W)
        self.concepts_var = tk.StringVar()
        ttk_bs.Entry(tags_frame, textvariable=self.concepts_var, width=70).pack(fill=X, pady=(5, 0))
    ...
    def create_exercise(self):
        ...
        concepts = [concept.strip() for concept in concepts_text.split(",") if concept.strip()] if concepts_text else []
