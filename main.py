...
class VisualLearningTracker:
    ...
    def create_sidebar(self, parent):
        ...
        for text, command in nav_buttons:
            btn = ttk_bs.Button(
                sidebar, text=text, command=command, style="Outline.TButton", width=25
            )
        ...
        quick_label = ttk_bs.Label(
            sidebar, text="Quick Actions", style="Subheader.TLabel"
        )
        ...
        timer_label = ttk_bs.Label(
            sidebar, text="Study Timer", style="Subheader.TLabel"
        )
        ...
        self.timer_display = ttk_bs.Label(
            self.timer_frame, text="00:00:00", font=("Helvetica", 14, "bold")
        )
    ...
    def create_active_courses_grid(self, parent):
        ...
        scrollbar = ttk_bs.Scrollbar(
            courses_frame, orient=VERTICAL, command=canvas.yview
        )
    ...
    def create_course_card(self, parent, course, row):
        ...
        # Title and category
        title_label = ttk_bs.Label(
            info_frame, text=course["title"], style="Subheader.TLabel"
        )
        title_label.pack(anchor=W)

        if course["category"]:
            category_label = ttk_bs.Label(
                info_frame, text=f"📂 {course['category']}", foreground="gray"
            )
    ...
    def start_study_session(self, course):
        if self.current_session_id:
            messagebox.showwarning(
                "Active Session", "Please end your current study session first."
            )
            return
    ...
    def refresh_dashboard(self):
        if (
            ...
            and self.notebook.tab("current")["text"] == "Dashboard"
        ):
            self.show_dashboard()
...
class QuickNoteDialog:
    ...
    def create_form(self):
        ...
        # Title
        ttk_bs.Label(main_frame, text="Create Quick Note", style="Header.TLabel").pack(
            pady=(0, 20)
        )
        ...
        ttk_bs.Button(
            btn_frame, text="Save Note", command=self.save_note, style="Success.TButton"
        ).pack(side=RIGHT)
