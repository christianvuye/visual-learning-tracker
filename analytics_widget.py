...
class AnalyticsWidget:
    ...
    def setup_ui(self):
        ...
        ttk_bs.Label(
            header_frame, text="📊 Learning Analytics", font=("Helvetica", 18, "bold")
        ).pack(side=LEFT)
    ...
    def create_stats_cards(self, parent):
        ...
        cards_data = [
            ...
            (
                "⏱️ Study Hours",
                f"{stats['total_study_hours']:.1f}h",
                "success",
                "hours",
            ),
    ...
    def create_stat_card(self, parent, title, value, style):
        ...
        # Icon and title
        ttk_bs.Label(card, text=title, font=("Helvetica", 10), foreground="gray").pack()

        # Value
        ttk_bs.Label(card, text=value, font=("Helvetica", 20, "bold")).pack(pady=(5, 0))
    ...
    def create_insights_section(self, parent):
        ...
        if insights:
            for insight in insights:
                ...
                ttk_bs.Label(
                    insight_item, text=insight, font=("Helvetica", 10), wraplength=600
    ...
    def create_progress_tab(self):
        ...
        progress_section = ttk_bs.LabelFrame(
            progress_frame, text="Course Progress", padding=15
        )
        ...
        if courses:
            ...
            scrollbar = ttk_bs.Scrollbar(
                progress_section, orient=VERTICAL, command=canvas.yview
            )
    ...
    def create_course_progress_item(self, parent, course):
        ...
        ttk_bs.Label(
            header_frame, text=course["title"], font=("Helvetica", 12, "bold")
        ).pack(side=LEFT)

        if course["category"]:
            ttk_bs.Label(
                header_frame, text=f"📂 {course['category']}", foreground="gray"
            ).pack(side=RIGHT)
        ...
        progress_label = ttk_bs.Label(
            progress_frame, text=f"{course['current_progress']:.1f}%"
        )
        ...
        ttk_bs.Label(
            stats_frame, text=stats_text, foreground="gray", font=("Helvetica", 9)
        ).pack(side=LEFT)
    ...
    def create_time_chart(self, parent):
        ...
        # Styling
        ax.set_title(
            "Study Hours This Week", color="white", fontsize=14, fontweight="bold"
        )
    ...
    def generate_insights(self):
        ...
        if stats["total_study_hours"] > 0:
            ...
            insights.append(
                f"You've studied {stats['total_study_hours']:.1f} hours in the {period_text} (avg: {avg_daily:.1f}h/day)"
            )

            if avg_daily >= 2:
                insights.append(
                    "🎉 Excellent! You're maintaining a great study routine."
                )
            elif avg_daily >= 1:
                insights.append(
                    "👍 Good progress! Consider increasing study time for faster progress."
                )
            else:
                insights.append(
                    "💡 Try to aim for at least 1 hour of study per day for consistent progress."
                )
        ...
        if stats["study_sessions"] > 0:
            if stats["avg_session_length"] > 0:
                insights.append(
                    f"Your average study session is {stats['avg_session_length']:.1f} minutes"
                )

                if stats["avg_session_length"] > 60:
                    insights.append(
                        "⚡ Great focus! Long study sessions show deep engagement."
                    )
                elif stats["avg_session_length"] > 30:
                    insights.append(
                        "📚 Good session length! This is optimal for retention."
                    )
                else:
                    insights.append(
                        "🔥 Consider longer sessions for deeper learning (30-60 minutes ideal)."
                    )

        # Course insights
        if stats["active_courses"] > 3:
            insights.append(
                "📚 You're learning multiple topics! Consider focusing on 2-3 courses for better progress."
            )
        elif stats["active_courses"] == 0:
            insights.append(
                "🚀 Ready to start learning? Create your first course to begin!"
            )

        # Module completion insights
        if stats["completed_modules"] > 0:
            insights.append(
                f"🎯 You've completed {stats['completed_modules']} modules in the {period_text}"
            )
