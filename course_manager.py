...
class CourseManagerWidget:
    ...
    def create_course_list(self):
        ...
        ttk_bs.Label(
            header_frame, text="📚 My Courses", font=("Helvetica", 16, "bold")
        ).pack(side=LEFT)
        ...
        scrollbar = ttk_bs.Scrollbar(
            self.courses_container, orient=VERTICAL, command=canvas.yview
        )
    ...
    def create_course_info_section(self, parent):
        ...
        for label_text, key in info_items:
            # Label
            ttk_bs.Label(
                info_frame, text=f"{label_text}:", font=("Helvetica", 10, "bold")
        ...
        # Description
        ttk_bs.Label(
            info_frame, text="Description:", font=("Helvetica", 10, "bold")
        ).grid(row=row, column=0, sticky=NW, padx=(0, 10), pady=(10, 0))

        self.description_text = tk.Text(
            info_frame, height=3, width=50, font=("Helvetica", 10), state="disabled"
        )
    ...
    def create_modules_section(self, parent):
        ...
        ttk_bs.Label(
            modules_header, text="📋 Learning Modules", font=("Helvetica", 12, "bold")
        ).pack(side=LEFT)
        ...
        self.modules_tree = ttk_bs.Treeview(
            modules_frame, columns=columns, show="tree headings", height=8
        )
        ...
        modules_scrollbar = ttk_bs.Scrollbar(
    ...
    def create_progress_section(self, parent):
        ...
        self.progress_bar = ttk_bs.Progressbar(
            progress_container, style="Success.Horizontal.TProgressbar", length=400
        )
        ...
        for i, (label_text, key) in enumerate(stats_items):
            ...
            ttk_bs.Label(stat_frame, text=label_text, font=("Helvetica", 9)).pack()

            value_label = ttk_bs.Label(
                stat_frame, text="0", font=("Helvetica", 14, "bold")
            )
            value_label.pack()
    ...
    def create_course_card(self, course):
        ...
        title_label = ttk_bs.Label(
            title_frame, text=course["title"], font=("Helvetica", 12, "bold")
        )
        ...
        if course["category"]:
            category_label = ttk_bs.Label(
                title_frame, text=f"📂 {course['category']}", foreground="gray"
            )
        ...
        progress_text = ttk_bs.Label(
            progress_frame, text=f"{course['current_progress']:.0f}%"
        )
    ...
    def load_course_details(self, course):
        ...
        self.info_labels["estimated_hours"].config(
            text=f"{course['estimated_hours']} hours"
        )
        self.info_labels["current_progress"].config(
        ...
        self.description_text.insert(
            1.0, course["description"] or "No description available."
        )
    ...
    def load_course_modules(self, course_id):
        ...
        for module in modules:
            ...
            estimated = (
                f"{module['estimated_minutes']}m"
                if module["estimated_minutes"]
                else "N/A"
            )
            actual = (
                f"{module['actual_minutes']}m" if module["actual_minutes"] else "N/A"
            )
        ...
        self.stats_labels["last_session"].config(
            text="Never"
    ...
    def start_study_session(self):
        if not self.current_course_id:
            ...
            return
        messagebox.showinfo(
            "Study Session", "Study session feature integrated with main app timer!"
        )
...
class ModuleDialog:
    ...
    def create_form(self):
        ...
        # Title
        ttk_bs.Label(main_frame, text="Add New Module", style="Header.TLabel").pack(
            pady=(0, 20)
        )
        ...
        ttk_bs.Entry(main_frame, textvariable=self.minutes_var, width=20).pack(
            anchor=W, pady=(0, 20)
        )
...
class CourseDialog:
    ...
    def create_form(self):
        ...
        # Title
        ttk_bs.Label(main_frame, text="Create New Course", style="Header.TLabel").pack(
            pady=(0, 20)
        )
        ...
        ttk_bs.Entry(main_frame, textvariable=self.title_var, width=50).pack(
            fill=X, pady=(0, 10)
        )
        ...
        difficulty_scale = ttk_bs.Scale(
            main_frame, from_=1, to=5, variable=self.difficulty_var, orient=HORIZONTAL
        )
        difficulty_scale.pack(fill=X, pady=(0, 10))

        self.hours_var = tk.IntVar()
        ttk_bs.Label(main_frame, text="Estimated Hours:").pack(anchor=W)
        ttk_bs.Entry(main_frame, textvariable=self.hours_var, width=20).pack(
            anchor=W, pady=(0, 20)
        )
