...
class MindMapNode:
    def __init__(self, x: float, y: float, text: str, node_id: str = None, 
                 color: str = "#3498db", node_type: str = "concept"):
        self.x = x
        self.y = y
        self.text = text
...
class MindMapConnection:
    def __init__(self, source_id: str, target_id: str, connection_type: str = "related",
                 color: str = "#95a5a6", label: str = ""):
...
class MindMapCanvas:
    ...
    def setup_canvas(self):
        ...
        # Scrollbars
        v_scrollbar = ttk_bs.Scrollbar(self.frame, orient=VERTICAL, command=self.canvas.yview)
        h_scrollbar = ttk_bs.Scrollbar(self.frame, orient=HORIZONTAL, command=self.canvas.xview)
    ...
    def show_context_menu(self, event, node):
        ...
        context_menu.add_command(label="Edit Text", command=lambda: self.edit_node_text(node))
        context_menu.add_command(label="Change Color", command=lambda: self.change_node_color(node))
    ...
    def create_connection(self, source: MindMapNode, target: MindMapNode) -> MindMapConnection:
    ...
    def change_node_color(self, node: MindMapNode = None):
        ...
        color = colorchooser.askcolor(title="Choose Node Color", initialcolor=target_node.color)
        if color[1]:  # If user didn't cancel
            target_node.color = color[1]
            self.redraw()
            
    def add_node_mode(self):
        messagebox.showinfo("Add Node", "Double-click anywhere on the canvas to add a new node.")
        
    def toggle_connection_mode(self):
        if not self.selected_node:
            messagebox.showwarning("No Selection", "Please select a node first, then click this button.")
            return
            
        self.connecting_mode = True
        self.connection_source = self.selected_node
        self.canvas.config(cursor="crosshair")
        messagebox.showinfo("Connection Mode", "Click on another node to create a connection.")
        
    def delete_selected(self):
        if self.selected_node:
            if messagebox.askyesno("Confirm Delete", f"Delete node '{self.selected_node.text}'?"):
    ...
    def draw_connection(self, connection: MindMapConnection):
        ...
        source_x, source_y = source_node.get_connection_point(target_node.x, target_node.y)
        target_x, target_y = target_node.get_connection_point(source_node.x, source_node.y)
