...
class NotesEditor:
    ...
    def create_notes_list(self):
        ...
        ttk_bs.Label(header_frame, text="📝 Notes", font=("Helvetica", 14, "bold")).pack(
            side=LEFT
        )
        ...
        ttk_bs.Label(search_frame, text="Search notes:", font=("Helvetica", 9)).pack(
            anchor=W
        )

        search_entry = ttk_bs.Entry(search_frame, textvariable=self.search_var)
        ...
        scrollbar = ttk_bs.Scrollbar(
            list_container, orient=VERTICAL, command=self.notes_tree.yview
        )
    ...
    def create_editor_toolbar(self, parent):
        ...
        ttk_bs.Separator(toolbar_frame, orient=VERTICAL).pack(
    ...
    def create_note_metadata(self, parent):
        ...
        self.title_entry = ttk_bs.Entry(
            title_frame, textvariable=self.title_var, font=("Helvetica", 12, "bold")
        )
        ...
        self.course_combo = ttk_bs.Combobox(
            course_frame, textvariable=self.course_var, state="readonly"
        )
        ...
        ttk_bs.Checkbutton(
            type_frame, text="⭐ Favorite", variable=self.favorite_var
        ).pack(side=LEFT)
    ...
    def create_text_editor(self, parent):
        ...
        self.text_editor = tk.Text(
            editor_frame, wrap=tk.WORD, font=("Helvetica", 12), undo=True, maxundo=50
        )

        # Scrollbar
        scrollbar = ttk_bs.Scrollbar(
            editor_frame, orient=VERTICAL, command=self.text_editor.yview
        )
    ...
    def create_note_actions(self, parent):
        ...
        tags_entry = ttk_bs.Entry(tags_container, textvariable=self.tags_var)
    ...
    def setup_text_tags(self):
        ...
        self.text_editor.tag_configure(
            "code", font=("Courier", 11), background="#f8f8f8"
        )
            ...
            if selected_text:
                ...
                if dialog.result:
                    ...
                    if course_name:
                        courses = self.db.get_courses()
                        course = next(
                            (c for c in courses if c["title"] == course_name), None
                        )
                        if course:
                            course_id = course["id"]
                    ...
                    if course_id:
                        self.db.create_flashcard(
                            course_id, question, answer, self.current_note_id
                        )
                        messagebox.showinfo("Success", "Flashcard created!")
                    else:
                        messagebox.showwarning(
                            "Warning", "Please select a course for this note."
                        )
            else:
    ...
    def add_to_mind_map(self):
        ...
        messagebox.showinfo(
            "Mind Map",
            f"Mind map integration coming soon!\nSelected text: {selected_text[:50]}...",
        )
...
class FlashcardDialog:
    ...
    def create_form(self, initial_text: str):
        ...
        ttk_bs.Label(
            main_frame, text="Create Flashcard", font=("Helvetica", 14, "bold")
        ).pack(pady=(0, 20))
        ...
        ttk_bs.Button(
            btn_frame, text="Create", command=self.create, style="Success.TButton"
        ).pack(side=RIGHT)
