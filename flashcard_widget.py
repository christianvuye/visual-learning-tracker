...
class FlashcardWidget:
    ...
    def create_controls(self):
        # Controls frame
        controls_frame = ttk_bs.LabelFrame(self.main_frame, text="Review Controls", padding=15)
    ...
    def create_statistics(self):
        # Statistics frame
        stats_frame = ttk_bs.LabelFrame(self.main_frame, text="Session Statistics", padding=15)
    ...
    def display_current_card(self):
        ...
        # Configure text tags
        self.card_content.tag_configure("question_header", font=('Helvetica', 12, 'bold'), foreground='blue')
        self.card_content.tag_configure("answer_header", font=('Helvetica', 12, 'bold'), foreground='green')
        self.card_content.tag_configure("question", font=('Helvetica', 14))
        self.card_content.tag_configure("answer", font=('Helvetica', 14), foreground='dark green')
        
        # Update navigation buttons
        self.prev_btn.config(state='normal' if self.current_card_index > 0 else 'disabled')
        self.next_btn.config(state='normal' if self.current_card_index < len(self.current_cards) - 1 else 'disabled')
        
    def display_no_cards_message(self):
        self.counter_label.config(text="No cards loaded")
        self.card_content.config(state='normal')
        self.card_content.delete(1.0, tk.END)
        self.card_content.insert(tk.END, "No flashcards available.\n\nClick 'Load Cards' to select a course or 'Add Card' to create new flashcards.")
        self.card_content.config(state='disabled')
    ...
    def rate_card(self, rating):
        ...
        if self.current_card_index < len(self.current_cards) - 1:
            ...
            messagebox.showinfo("Session Complete", f"Great job! You've reviewed all {len(self.current_cards)} cards.")
    ...
    def update_statistics(self):
        ...
        # Calculate accuracy
        accuracy = (self.correct_answers / self.cards_reviewed * 100) if self.cards_reviewed > 0 else 0
...
class CourseSelectionDialog:
    def __init__(self, parent, courses):
        ...
        ttk_bs.Label(main_frame, text="Select a course to study:", font=('Helvetica', 12, 'bold')).pack(pady=(0, 15))
...
class AddCardDialog:
    ...
    def create_form(self, courses):
        ...
        ttk_bs.Label(main_frame, text="Create New Flashcard", font=('Helvetica', 14, 'bold')).pack(pady=(0, 20))
