...
class KnowledgeGraphWidget:
    ...
    def create_graph_panel(self):
        ...
        self.ax.set_title('Knowledge Graph Visualization', color='white', fontsize=14, fontweight='bold')
    ...
    def create_node_details(self, parent):
        ...
        # Node description
        ttk_bs.Label(details_frame, text="Description:", font=('Helvetica', 10, 'bold')).pack(anchor=W, pady=(10, 5))
    ...
    def create_graph_stats(self, parent):
        ...
        for label_text, key in stats_items:
            ...
            value_label = ttk_bs.Label(stat_item, text="0", font=('Helvetica', 10, 'bold'))
    ...
    def draw_graph(self):
        ...
        if len(self.graph.nodes()) == 0:
            self.ax.text(0.5, 0.5, 'No nodes in graph\nClick "Add Node" to get started',
    ...
    def update_statistics(self):
        ...
        num_clusters = nx.number_connected_components(self.graph) if num_nodes > 0 else 0
    ...
    def connect_nodes_dialog(self):
        ...
        if len(self.graph.nodes()) < 2:
            messagebox.showwarning("Insufficient Nodes", "Need at least 2 nodes to create connections.")
            return
    ...
    def delete_selected_node(self):
        ...
        if messagebox.askyesno("Confirm Delete", f"Delete node '{self.selected_node}'?"):
    ...
    def analyze_graph(self):
        ...
        analysis.append(f"Graph contains {num_nodes} concepts with {num_edges} connections.")
        ...
        if isolated:
            analysis.append(f"Isolated concepts: {', '.join(isolated)} (need more connections)")
    ...
    def load_course_graph(self):
        ...
        if not courses:
            ...
            return
        messagebox.showinfo("Load Course", "Course-specific knowledge graphs coming soon!")
    ...
    def new_graph(self):
        ...
        if len(self.graph.nodes()) > 0:
            if not messagebox.askyesno("New Graph", "This will clear the current graph. Continue?"):
                return
            ...
            try:
                ...
                graph_data = {
                    'nodes': [
                        {
                            'id': node,
                            'node_type': self.graph.nodes[node].get('node_type', 'concept'),
...
class AddNodeDialog:
    ...
    def create_form(self):
        ...
        ttk_bs.Label(main_frame, text="Add Knowledge Node", font=('Helvetica', 14, 'bold')).pack(pady=(0, 20))
        
        # Title
        ttk_bs.Label(main_frame, text="Concept Title:").pack(anchor=W)
        self.title_var = tk.StringVar()
        ttk_bs.Entry(main_frame, textvariable=self.title_var, width=40).pack(fill=X, pady=(5, 15))
...
class ConnectNodesDialog:
    ...
    def create_form(self, nodes):
        ...
        ttk_bs.Label(main_frame, text="Connect Knowledge Nodes", font=('Helvetica', 14, 'bold')).pack(pady=(0, 20))
        
        # Source node
        ttk_bs.Label(main_frame, text="Source Node:").pack(anchor=W)
        self.source_var = tk.StringVar()
        source_combo = ttk_bs.Combobox(main_frame, textvariable=self.source_var, values=nodes, state="readonly")
        source_combo.pack(fill=X, pady=(5, 15))
        
        # Target node
        ttk_bs.Label(main_frame, text="Target Node:").pack(anchor=W)
        self.target_var = tk.StringVar()
        target_combo = ttk_bs.Combobox(main_frame, textvariable=self.target_var, values=nodes, state="readonly")
...
class EditNodeDialog:
    ...
    def create_form(self, current_title, node_data):
        ...
        ttk_bs.Label(main_frame, text="Edit Knowledge Node", font=('Helvetica', 14, 'bold')).pack(pady=(0, 20))
